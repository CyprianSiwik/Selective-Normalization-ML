...
def run_selective_norm(model_type='cnn', dataset='cifar10', dropout_rate=0.3, normalization='selective'):
    ...
    Args:
        ...
        normalization (str): must be 'selective' here
    """
    if normalization != 'selective':
        raise ValueError("Normalization must be set to 'selective' for selective_norm method.")
    ...
    # === Model Selection ===
    if model_type == 'cnn':
        model = CNN(input_channels=input_channels, num_classes=num_classes, dropout=dropout_rate, norm='selective')

    elif model_type == 'mlp':
        model = MLP(input_size=input_size, hidden_sizes=[512, 256], num_classes=num_classes, dropout=dropout_rate, norm='selective')
